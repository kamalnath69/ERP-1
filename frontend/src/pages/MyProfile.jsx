...
import {
  applyApiErrors, FORM_OPTIONS, mfaSensitiveSchema, mfaStartSchema, mfaVerifySchema,
  myProfileSchema, passwordChangeSchema,
} from "@/lib/validation";
import {
...
export default function MyProfile() {
  ...
  const profileForm = useForm({ resolver: zodResolver(myProfileSchema), defaultValues: profileValues(), ...FORM_OPTIONS });
  const passwordForm = useForm({ resolver: zodResolver(passwordChangeSchema), defaultValues: passwordValues(), ...FORM_OPTIONS });
  const mfaStartForm = useForm({ resolver: zodResolver(mfaStartSchema), defaultValues: { current_password: "" }, ...FORM_OPTIONS });
  const mfaVerifyForm = useForm({ resolver: zodResolver(mfaVerifySchema), defaultValues: { code: "" }, ...FORM_OPTIONS });
  const mfaSensitiveForm = useForm({ resolver: zodResolver(mfaSensitiveSchema), defaultValues: { current_password: "", code: "" }, ...FORM_OPTIONS });
  const profileAvatar = profileForm.watch("avatar_base64");
  const newPassword = passwordForm.watch("new_password") || "";
  const sessionPending = usePendingAction();
  const preferencePending = usePendingAction();
  ...
  const saveProfile = profileForm.handleSubmit(async (values) => {
    profileForm.clearErrors("root.server");
    try {
      ...
      const normalized = applyApiErrors(error, profileForm.setError, { fallback: "Profile could not be updated" });
      if (!Object.keys(normalized.fieldErrors).length) profileForm.setError("root.server", { type: "server", message: normalized.message });
    }
  });
  const saveAppearance = async (mode) => {
    await preferencePending.run("appearance", async () => {
  ...
  const saveNotifications = async (next) => {
    await preferencePending.run("notifications", async () => {
  ...
  const submitPassword = passwordForm.handleSubmit(async (values) => {
    ...
    try {
      ...
      const normalized = applyApiErrors(error, passwordForm.setError, { fallback: "Password could not be updated" });
      if (!Object.keys(normalized.fieldErrors).length) passwordForm.setError("root.server", { type: "server", message: normalized.message });
    }
  });

  const beginEnrollment = mfaStartForm.handleSubmit(async (values) => {
    mfaStartForm.clearErrors("root.server");
    try {
      const result = await startMfa(values).unwrap();
      setEnrollment(result);
      mfaVerifyForm.reset({ code: "" });
    } catch (error) {
      const normalized = applyApiErrors(error, mfaStartForm.setError, { fallback: "Authenticator setup could not start" });
      if (!Object.keys(normalized.fieldErrors).length) mfaStartForm.setError("root.server", { type: "server", message: normalized.message });
    }
  });
  const completeEnrollment = mfaVerifyForm.handleSubmit(async (values) => {
    mfaVerifyForm.clearErrors("root.server");
    try {
      const result = await verifyMfa(values).unwrap();
      setRecoveryCodes(result.recovery_codes || []);
      setEnrollment(null);
      security.refetch();
      toast.success("Authenticator security enabled");
    } catch (error) {
      const normalized = applyApiErrors(error, mfaVerifyForm.setError, { fallback: "The authenticator code was not accepted" });
      if (!Object.keys(normalized.fieldErrors).length) mfaVerifyForm.setError("root.server", { type: "server", message: normalized.message });
    }
  });
  ...
  const sensitiveMfaAction = mfaSensitiveForm.handleSubmit(async (values) => {
    mfaSensitiveForm.clearErrors("root.server");
    try {
      if (mfaDialog === "disable") {
        ...
        const result = await regenerateCodes(values).unwrap();
        setRecoveryCodes(result.recovery_codes || []);
        setMfaDialog("recovery");
        security.refetch();
      }
    } catch (error) {
      const normalized = applyApiErrors(error, mfaSensitiveForm.setError, { fallback: "Security settings could not be changed" });
      if (!Object.keys(normalized.fieldErrors).length) mfaSensitiveForm.setError("root.server", { type: "server", message: normalized.message });
    }
  });
  ...
  const openMfa = (mode) => {
    setRecoveryCodes([]);
    setEnrollment(null);
    mfaStartForm.reset({ current_password: "" });
    mfaVerifyForm.reset({ code: "" });
    mfaSensitiveForm.reset({ current_password: "", code: "" });
    setMfaDialog(mode);
  };
  ...
  return <PageShell size="standard" className="reveal" data-testid="my-profile-page">
    ...
    {user.mfa_enrollment_required && <Surface className="border-warning/40 bg-warning-soft p-5"><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div className="flex gap-3"><ShieldCheck className="shrink-0 text-warning" size={24} /><div><div className="font-semibold">Authenticator setup is required</div><p className="mt-1 text-sm text-muted-foreground">Your organization requires an additional sign-in code for this account.</p></div></div><Button onClick={() => { setTab("security"); openMfa("enroll"); }}>Set up now</Button></div></Surface>}

    <DetailHero
      avatar={<div className="grid h-20 w-20 place-items-center overflow-hidden rounded-3xl bg-primary font-display text-3xl text-primary-foreground">{profileAvatar ? <img src={profileAvatar} alt="Your profile" className="h-full w-full object-cover" /> : initials}</div>}
    ...
    <Tabs value={activeTab} onValueChange={setTab}>
      ...
      <TabsContent value="profile" className="mt-5"><Surface className="p-5 sm:p-7"><SectionTitle title="Profile details" copy="These details help your team identify and contact you." /><Form {...profileForm}><form noValidate onSubmit={saveProfile} className="mt-6 space-y-6"><AvatarUploader value={profileAvatar} onChange={(value) => profileForm.setValue("avatar_base64", value, { shouldDirty: true, shouldValidate: true })} initials={initials} disabled={profileForm.formState.isSubmitting || profileState.isLoading} /><FormMessageFor control={profileForm.control} name="avatar_base64" /><div className="grid gap-4 sm:grid-cols-2"><ValidatedField control={profileForm.control} name="first_name" label="First name"><Input data-testid="profile-first-name" autoComplete="given-name" /></ValidatedField><ValidatedField control={profileForm.control} name="last_name" label="Last name"><Input autoComplete="family-name" /></ValidatedField><ValidatedField control={profileForm.control} name="phone" label="Phone"><Input data-testid="profile-phone" inputMode="tel" autoComplete="tel" /></ValidatedField><ValidatedField control={profileForm.control} name="designation" label="Job title"><Input data-testid="profile-designation" placeholder={isCollege ? "Placement head, HOD, faculty..." : "Studio manager, senior trainer..."} /></ValidatedField></div><ValidatedField control={profileForm.control} name="bio" label="About me"><Textarea rows={4} data-testid="profile-bio" placeholder="A short introduction for your team" /></ValidatedField><FormRootError error={profileForm.formState.errors.root?.server} /><div className="flex justify-end"><Button type="submit" data-testid="profile-save-btn" loading={profileForm.formState.isSubmitting || profileState.isLoading} loadingText="Saving profile..."><FloppyDisk className="mr-2" />Save profile</Button></div></form></Form></Surface></TabsContent>
      ...
      <TabsContent value="security" className="mt-5 space-y-5"><SecurityPanel state={security} onEnroll={() => openMfa("enroll")} onCodes={() => openMfa("regenerate")} onDisable={() => openMfa("disable")} /><SessionsPanel query={sessions} pending={sessionPending.isPending} revoke={(session) => sessionPending.run(`session:${session.id}`, async () => { try { const result = await revokeSession(session.id).unwrap(); if (result.signed_out) return navigate("/login", { replace: true }); toast.success("Device signed out"); } catch (error) { toast.error(error?.data?.detail || "Device could not be signed out"); } })} revokeAll={() => sessionPending.run("sessions:all", async () => { try { await revokeAll().unwrap(); navigate("/login", { replace: true }); } catch { toast.error("Sessions could not be closed"); } })} /><Surface className="p-5 sm:p-6"><SectionTitle title="Change password" copy="Changing your password closes every existing session." /><Form {...passwordForm}><form noValidate onSubmit={submitPassword}><div className="mt-5 grid gap-4 md:grid-cols-3"><ValidatedField control={passwordForm.control} name="current_password" label="Current password"><Input data-testid="pw-current" type="password" autoComplete="current-password" /></ValidatedField><div><ValidatedField control={passwordForm.control} name="new_password" label="New password"><Input data-testid="pw-new" type="password" autoComplete="new-password" /></ValidatedField><PasswordStrength password={newPassword} compact /></div><ValidatedField control={passwordForm.control} name="confirm" label="Confirm new password"><Input data-testid="pw-confirm" type="password" autoComplete="new-password" /></ValidatedField></div><FormRootError className="mt-4" error={passwordForm.formState.errors.root?.server} /><div className="mt-5 flex justify-end"><Button type="submit" data-testid="pw-save-btn" variant="outline" loading={passwordForm.formState.isSubmitting || passwordState.isLoading} loadingText="Updating password...">Update password</Button></div></form></Form></Surface></TabsContent>
    </Tabs>

    <AssistantPersonalizationSheet open={personalizationOpen} onOpenChange={setPersonalizationOpen} />

    <Dialog open={Boolean(mfaDialog)} onOpenChange={(open) => { const busy = startMfaState.isLoading || verifyMfaState.isLoading || disableState.isLoading || regenerateState.isLoading; if (!open && !recoveryCodes.length && !busy) { setMfaDialog(null); setEnrollment(null); } }}><DialogContent className="sm:max-w-lg"><DialogHeader><DialogTitle className="font-display text-3xl">{mfaDialog === "disable" ? "Turn off authenticator security" : mfaDialog === "regenerate" ? "Replace recovery codes" : "Set up authenticator security"}</DialogTitle></DialogHeader>
      {recoveryCodes.length ? <RecoveryCodes codes={recoveryCodes} finish={mfaDialog === "enroll" ? finishEnrollment : () => { setRecoveryCodes([]); setMfaDialog(null); }} /> : mfaDialog === "enroll" ? !enrollment ? <Form {...mfaStartForm}><form noValidate onSubmit={beginEnrollment} className="space-y-5"><p className="text-sm leading-6 text-muted-foreground">Confirm your password before connecting an authenticator app.</p><ValidatedField control={mfaStartForm.control} name="current_password" label="Current password"><Input autoFocus type="password" autoComplete="current-password" /></ValidatedField><FormRootError error={mfaStartForm.formState.errors.root?.server} /><Button type="submit" className="w-full" loading={mfaStartForm.formState.isSubmitting || startMfaState.isLoading} loadingText="Preparing...">Continue</Button></form></Form> : <Form {...mfaVerifyForm}><form noValidate onSubmit={completeEnrollment} className="space-y-5"><p className="text-sm leading-6 text-muted-foreground">Open your authenticator app, add an account using the setup key below, then enter its six-digit code.</p><div className="rounded-2xl border bg-surface-subtle p-4"><div className="text-xs text-muted-foreground">Setup key</div><div className="mt-2 break-all font-mono text-lg font-semibold tracking-wider">{enrollment.secret}</div><Button className="mt-3" size="sm" variant="outline" onClick={() => navigator.clipboard.writeText(enrollment.secret)}><Copy className="mr-2" />Copy key</Button></div><a href={enrollment.provisioning_uri} className="block text-center text-sm font-semibold text-accent sm:hidden">Open authenticator app</a><ValidatedField control={mfaVerifyForm.control} name="code" label="6-digit code"><Input autoFocus inputMode="numeric" autoComplete="one-time-code" /></ValidatedField><FormRootError error={mfaVerifyForm.formState.errors.root?.server} /><Button type="submit" className="w-full" loading={mfaVerifyForm.formState.isSubmitting || verifyMfaState.isLoading} loadingText="Verifying...">Verify and enable</Button></form></Form> : <Form {...mfaSensitiveForm}><form noValidate onSubmit={sensitiveMfaAction} className="space-y-5"><p className="text-sm leading-6 text-muted-foreground">Confirm with your password and a current authenticator or recovery code.</p><ValidatedField control={mfaSensitiveForm.control} name="current_password" label="Current password"><Input type="password" autoComplete="current-password" /></ValidatedField><ValidatedField control={mfaSensitiveForm.control} name="code" label="Authenticator or recovery code"><Input autoComplete="one-time-code" /></ValidatedField><FormRootError error={mfaSensitiveForm.formState.errors.root?.server} /><Button type="submit" className="w-full" variant={mfaDialog === "disable" ? "destructive" : "default"} loading={mfaSensitiveForm.formState.isSubmitting || disableState.isLoading || regenerateState.isLoading} loadingText="Confirming...">{mfaDialog === "disable" ? "Turn off and sign out" : "Create new codes"}</Button></form></Form>}
...
function SessionsPanel({ query, revoke, revokeAll, pending }) {
  return <Surface className="p-5 sm:p-6"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><SectionTitle title="Signed-in devices" copy="Close any session you do not recognize." /><Button variant="outline" loading={pending("sessions:all")} loadingText="Signing out..." onClick={revokeAll}>Sign out everywhere</Button></div><div className="mt-5 divide-y">{query.isLoading ? [1, 2].map((item) => <div key={item} className="h-20 animate-pulse bg-surface-subtle" />) : query.data?.length ? query.data.map((session) => <div key={session.id} className="flex flex-col gap-3 py-4 first:pt-0 last:pb-0 sm:flex-row sm:items-center sm:justify-between"><div className="flex gap-3">{/mobile|android|iphone/i.test(session.user_agent || "") ? <DeviceMobile size={22} className="text-accent" /> : <Desktop size={22} className="text-accent" />}<div><div className="font-medium">{deviceName(session.user_agent)} {session.is_current && <StatusBadge className="ml-2" status="active" label="This device" />}</div><div className="mt-1 text-xs text-muted-foreground">Signed in {new Date(session.created_at).toLocaleString("en-IN")}</div></div></div><Button size="sm" variant="outline" loading={pending(`session:${session.id}`)} loadingText="Signing out..." onClick={() => revoke(session)}><SignOut className="mr-2" />Sign out</Button></div>) : <p className="text-sm text-muted-foreground">No other active devices.</p>}</div></Surface>;
}
...
function FormMessageFor({ control, name }) { return <FormField control={control} name={name} render={() => <FormItem><FormMessage /></FormItem>} />; }
function profileValues(user = {}) { return { first_name: user.first_name || "", last_name: user.last_name || "", phone: user.phone || "", designation: user.designation || "", bio: user.bio || "", avatar_base64: user.avatar_base64 || null }; }
function passwordValues() { return { current_password: "", new_password: "", confirm: "" }; }
