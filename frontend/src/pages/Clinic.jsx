...
function PrescriptionsPanel({ query, canSign }) {
  const [sign, state] = useSignPrescriptionMutation();
  const doSign = async (row) => { try { await sign(row.id).unwrap(); toast.success("Prescription signed and locked"); } catch (error) { toast.error(error?.data?.detail || "Prescription could not be signed"); } };
  ...
  const columns = [
    ...
    { key: "status", label: "State", render: (row) => <StatusBadge status={row.status} /> },
    { key: "action", label: "", render: (row) => row.status === "draft" && canSign ? <Button disabled={state.isLoading} size="sm" onClick={(event) => { event.stopPropagation(); doSign(row); }}>Review and sign</Button> : null },
  ];
...
function LabsPanel({ ordersQuery, canManage, canWrite, canSign, onCreate }) {
  const [sign, state] = useSignLabOrderMutation();
  const doSign = async (row) => { try { await sign(row.id).unwrap(); toast.success("Lab order signed"); } catch (error) { toast.error(error?.data?.detail || "Lab order could not be signed"); } };
  ...
  const columns = [
    ...
    { key: "status", label: "State", render: (row) => <StatusBadge status={row.status} /> },
    { key: "action", label: "", render: (row) => !row.signed_at && canSign ? <Button disabled={state.isLoading} size="sm" onClick={(event) => { event.stopPropagation(); doSign(row); }}>Sign order</Button> : null },
  ];
