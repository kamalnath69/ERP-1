...
export default function SuperAdmin() {
  ...
  return <div className="flex min-h-screen bg-background text-foreground">
    <aside className={`${navOpen ? "flex" : "hidden"} fixed inset-y-0 left-0 z-50 w-72 flex-col bg-sidebar text-sidebar-foreground lg:sticky lg:flex`}>
      <div className="flex h-20 items-center justify-between border-b border-sidebar-foreground/10 px-6"><button onClick={() => open("overview")} className="font-display text-2xl font-bold">Edvatiq</button><button className="lg:hidden" onClick={() => setNavOpen(false)}><X /></button></div>
...
function PlanAssignmentPanel({ organization, subscription, refresh }) {
  ...
  const assign = async () => { try { await api.post(`/super-admin/organizations/${organization.id}/plan`, { plan_version_id: planId, billing_interval: interval, change_timing: timing, reason, version: subscription?.version || 1, idempotency_key: crypto.randomUUID() }); toast.success(timing === "cycle_end" ? "Plan change scheduled" : "Plan updated"); refresh(); } catch (error) { toast.error(message(error)); } };
  const override = async () => { const feature = features.data.find((item) => item.code === featureCode); const value = feature?.value_type === "boolean" ? overrideValue === "true" : overrideValue === "" ? null : Number(overrideValue); try { await api.post(`/super-admin/organizations/${organization.id}/overrides`, { feature_code: featureCode, value, reason: overrideReason, starts_at: new Date().toISOString(), ends_at: new Date(Date.now() + 30 * 86400000).toISOString(), version: 1 }); toast.success("Temporary adjustment granted for 30 days"); refresh(); } catch (error) { toast.error(message(error)); } };
  return <><Panel title="Change plan"><FormSelect label="Plan version" value={planId} set={setPlanId} options={published} /><div className="grid grid-cols-2 gap-3"><FormSelect label="Billing" value={interval} set={setInterval} options={[["monthly", "Monthly"], ["annual", "Annual"]]} /><FormSelect label="When" value={timing} set={setTiming} options={[["immediate", "Immediately"], ["cycle_end", "At cycle end"]]} /></div><FieldLabel text="Business reason"><Input value={reason} onChange={(event) => setReason(event.target.value)} /></FieldLabel><Button className="w-full mt-4" disabled={!planId || reason.length < 5} onClick={assign}>Apply plan change</Button></Panel><Panel title="Temporary adjustment"><FormSelect label="Feature or limit" value={featureCode} set={setFeatureCode} options={features.data.map((item) => [item.code, item.name])} />{features.data.find((item) => item.code === featureCode)?.value_type === "boolean" ? <FormSelect label="Availability" value={overrideValue} set={setOverrideValue} options={[["true", "Included"], ["false", "Not included"]]} /> : <FieldLabel text="Temporary limit"><Input type="number" min="0" value={overrideValue} onChange={(event) => setOverrideValue(event.target.value)} placeholder="Leave blank for unlimited" /></FieldLabel>}<FieldLabel text="Reason"><Input value={overrideReason} onChange={(event) => setOverrideReason(event.target.value)} /></FieldLabel><Button variant="outline" className="w-full mt-4" disabled={!featureCode || overrideReason.length < 8} onClick={override}>Grant for 30 days</Button></Panel></>;
}
...
function PlanEditor({ version, features, onSaved }) {
  ...
  return <div className="space-y-5">
    ...
    <div className="sticky bottom-0 flex justify-end border-t bg-background py-4"><Button disabled={saving} onClick={save}>{saving ? "Saving..." : "Save draft"}</Button></div>
  </div>;
}
