...
import { applyApiErrors, documentUploadSchema, FORM_OPTIONS } from "@/lib/validation";
...
export default function Documents() {
  ...
  const columns = [
    ...
    { key: "actions", label: "", render: (row) => <div className="flex justify-end gap-1"><Button asChild size="sm" variant="ghost"><a href={`${API_BASE}/documents/${row.id}/download`} aria-label={`Download ${row.name}`}><DownloadSimple /></a></Button>{can("documents.manage") && knowledgeEnabled && <Button size="sm" variant="ghost" loading={rowActions.isPending(`reindex:${row.id}`)} disabled={row.status === "processing"} onClick={() => rowActions.run(`reindex:${row.id}`, async () => { try { await reindex(row.id).unwrap(); toast.success("Document queued for processing"); } catch (error) { toast.error(error?.data?.detail || "Could not reprocess document"); } })} aria-label={`Reprocess ${row.name}`}><ArrowClockwise /></Button>}{can("documents.manage") && <Button size="sm" variant="ghost" disabled={rowActions.isPending(`reindex:${row.id}`)} onClick={() => setDeleting(row)} aria-label={`Delete ${row.name}`}><Trash /></Button>}</div> },
  ];
  ...
  return <PageShell className="reveal">
    ...
    <AlertDialog open={Boolean(deleting)} onOpenChange={(open) => { if (!open && !removeState.isLoading) setDeleting(null); }}><AlertDialogContent><AlertDialogHeader><AlertDialogTitle>Delete this document?</AlertDialogTitle><AlertDialogDescription>The file and its searchable knowledge will be removed. This cannot be undone.</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel disabled={removeState.isLoading}>Keep document</AlertDialogCancel><Button variant="destructive" loading={removeState.isLoading} loadingText="Deleting..." onClick={async () => { try { await remove(deleting.id).unwrap(); toast.success("Document deleted"); setDeleting(null); } catch (error) { toast.error(error?.data?.detail || "Could not delete document"); } }}>Delete document</Button></AlertDialogFooter></AlertDialogContent></AlertDialog>
  </PageShell>;
}

function LegacyUploadDrawer({ open, onOpenChange, locationId }) {
...
function UploadDrawer({ open, onOpenChange, locationId }) {
  ...
  const form = useForm({
    ...
    defaultValues: { file: null, visibility: "team" },
    ...FORM_OPTIONS,
  });
  const { clearErrors, formState, handleSubmit, register, reset, setError, setValue, watch } = form;
  const file = watch("file");
  const visibility = watch("visibility");
  const close = (next) => {
    if (!next && (formState.isSubmitting || result.isLoading)) return;
    if (!next) { reset(); clearErrors(); }
    onOpenChange(next);
  };
  const submit = handleSubmit(async (values) => {
    ...
    try {
      ...
      const normalized = applyApiErrors(error, setError, { fallback: "Could not upload document" });
      toast.error(normalized.message);
    }
  });
  return <DrawerForm open={open} onOpenChange={close} title="Upload document" description="Files remain private and are checked before they become searchable."><form noValidate onSubmit={submit} className="space-y-5"><div className="rounded-2xl border border-dashed p-6 text-center"><UploadSimple className="mx-auto text-muted-foreground" size={30} /><Label htmlFor="document-file" className="mt-4 block cursor-pointer font-semibold">Choose PDF, DOCX, TXT, JPG, or PNG</Label><p className="mt-2 text-xs text-muted-foreground">Maximum 20 MB</p><Input id="document-file" className="mt-4" type="file" accept=".pdf,.docx,.txt,.jpg,.jpeg,.png" {...register("file")} onChange={(event) => setValue("file", event.target.files?.[0] || null, { shouldDirty: true, shouldTouch: true, shouldValidate: true })} aria-invalid={Boolean(formState.errors.file)} aria-describedby="document-file-error" />{file && <p className="mt-3 text-sm">{file.name} / {formatBytes(file.size)}</p>}<FieldError id="document-file-error" error={formState.errors.file} className="mt-2" /></div><div className="space-y-2"><Label htmlFor="document-visibility">Who can see it</Label><Select value={visibility} onValueChange={(value) => setValue("visibility", value, { shouldDirty: true, shouldValidate: true })}><SelectTrigger id="document-visibility" aria-invalid={Boolean(formState.errors.visibility)}><SelectValue /></SelectTrigger><SelectContent><SelectItem value="team">Team</SelectItem><SelectItem value="managers">Managers</SelectItem><SelectItem value="author_only">Only me</SelectItem></SelectContent></Select><FieldError id="document-visibility-error" error={formState.errors.visibility} /></div><FormRootError error={formState.errors.root?.server} /><Button type="submit" loading={formState.isSubmitting || result.isLoading} loadingText="Uploading securely..." className="w-full">Upload securely</Button></form></DrawerForm>;
}
